...
(function ($) {
  ...
  $.formUtils.addValidator({
    ...
    validatorFunction: function (val, $el, conf, lang) {
      ...
      switch (lengthCheckResults[0]) {   // outside of allowed range
        case "out":
          this.errorMessage = lang.lengthBadStart + lengthAllowed + lang.lengthBadEnd + "数字";
          checkResult = false;
          break;
        // too short
        case "min":
  ...
  /*
   * Validate select_required
   */
  $.formUtils.addValidator({
    name: 'select_required',
    validatorFunction: function (val, $el, config) {
      if ($el.val()==null) {
        var msg = $el.children('[value=0]').html();
        if (msg == '') {
          this.errorMessageKey = 'requiredFields';
        } else{
